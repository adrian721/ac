        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
              <h1 className="text-2xl sm:text-3xl font-black tracking-tighter leading-none flex items-center gap-1">
                <span>KOOL</span>
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            {currentUser.role === 'SUPER_ADMIN' && onNavigateToPermissions && (
              <button
                ...
                className="hidden lg:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-950/60 hover:bg-purple-900/80 text-purple-300 border border-purple-500/40 text-xs font-bold transition shadow-sm"
              >
            ...
            <div className="relative">
              ...
              {showRoleDropdown && (
                <>
                  ...
                  />
                  <div className="absolute right-0 mt-2 w-80 bg-[#121212] rounded-2xl shadow-2xl border border-white/15 z-50 p-2 divide-y divide-white/10 text-white animate-fadeIn">
                    
                    {/* User profile card */}
                    <div className="p-3 bg-slate-900/90 border border-slate-800 rounded-xl mb-2">
                      <div className="flex items-center gap-2.5">
                        <img src={currentUser.avatar} alt={currentUser.name} className="w-10 h-10 rounded-xl object-cover border border-white/20" />
                    ...
                    <div className="p-2 text-[10px] text-slate-400 font-semibold uppercase tracking-wider">
                      Beralih Akun (Simulasi Cepat):
                    </div>

                    <div className="py-1 space-y-1 max-h-56 overflow-y-auto pr-1">
                      {users.map(u => {
                        ...
                        return (
                          <button
                            key={u.id}
                            id={`dropdown-switch-user-${u.id}`}
                            onClick={() => {
                              switchRole(u.role, u.id);
                              setShowRoleDropdown(false);
                            }}
                            className={`w-full flex items-center justify-between p-2 rounded-xl text-left text-xs transition cursor-pointer ${
                              isCurrent ? 'bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-bold' : 'hover:bg-white/5 text-white/80'
                            }`}
                          >
                            <div className="flex items-center gap-2.5 min-w-0">
                              ...
                              <div className="truncate">
                                ...
                                <span className={`inline-flex items-center gap-1 text-[8px] font-black uppercase tracking-wider px-1.5 py-0.2 rounded border ${
                                  isCurrent ? 'bg-white/20 text-white border-white/30' : roleMeta?.color || ''
                                }`}>
                                  {roleMeta?.icon}
                                  {roleMeta?.title}
                                </span>
