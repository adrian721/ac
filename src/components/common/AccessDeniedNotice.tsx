...
import { ShieldAlert, Lock, ArrowLeft, RefreshCw, KeyRound, AlertTriangle } from 'lucide-react';
...
export const AccessDeniedNotice: React.FC<AccessDeniedNoticeProps> = ({ 
  ...
  const { currentUser, systemFeatureDefinitions, switchRole } = useApp();
      ...
      <div className="max-w-lg w-full bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl text-center space-y-5">
        ...
        <div className="space-y-2">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-rose-500/10 text-rose-300 border border-rose-500/20">
            <Lock className="w-3.5 h-3.5" /> Akses Dibatasi oleh Super Admin
          </span>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
          {onGoBack && (
            <button
              onClick={onGoBack}
              className="w-full sm:w-auto px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
            >
              <ArrowLeft className="w-4 h-4" /> Kembali ke Halaman Utama
            </button>
          )}

          {/* Quick Switch Button for demo convenience */}
          <button
            onClick={() => switchRole('SUPER_ADMIN')}
            className="w-full sm:w-auto px-4 py-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 shadow-md shadow-purple-600/30 transition-colors"
          >
            <RefreshCw className="w-4 h-4" /> Beralih ke Super Admin (Test)
          </button>
        </div>
